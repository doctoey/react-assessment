...
const Admin = ({employees,navigateToHome,navigateToUser,navigateToAdmin}) => {
    ...
    const handleDelete = (id) => { // Event handler for delete button
        const updatedEmployees = employeesList.filter(item => item.id !== id); // Filter out employee with the given id
        setEmployeesList(updatedEmployees); // Update employeesList state
      }
